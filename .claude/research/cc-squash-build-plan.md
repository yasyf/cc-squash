...
> **Finalize pass (2026-06-20):** every crate in §4 was re-resolved against crates.io on this date — all are real and maintained; `pingora 0.8.1` and `cargo-dist/dist 0.32.0` match exactly. **(The fusekit amendment subsequently dropped the Rust FUSE crates `fuser`/`fuse3` from §4 — FUSE is now the fusekit Go holder, §2/§6.)** **(Dedup pass 2026-06-22: cc-pool's `service`/`state`/`version` are promoted into fusekit — v0.5.1→v0.6.0, sibling to `proc`/`mountd` — and cc-pool is back-ported onto them; §1.5/§2/§5.)** Two version corrections were folded in: **rmcp** is `1.7.0` (the SDK reached 1.x; the inherited `0.16` was stale) and **failsafe** is `1.3.0` (API-stable since Jul 2024). cc-pool's live state (`~/.cc-pool`: `daemon.sock`/`daemon.sock.lock`/`pool.db{,-wal,-shm}`/`mount-holder.log`/`mounts.sock`/`status.json`) and the superseded Python scaffold (`cc_squash/`, `pyproject.toml`, `uv.lock`) were both confirmed on disk, anchoring the §1 daemon ports and the §9 supersession pitfall.
...
- **Distribution = one Homebrew formula installing both binaries, co-released under one git tag.** The Go `ccs` is built/released by **goreleaser** (a cc-squash choice — cc-pool itself uses a hand-rolled `release.yml` heredoc, not goreleaser); the Rust `ccs-proxy` by **cargo-dist** (`dist` 0.32). A single generated formula (installing both `ccs` and `ccs-proxy`) is written **at release into the external shared tap** (`yasyf/homebrew-tap`, the cc-pool pattern — the formula lives in the tap, *not* in the project repo, populated by the release workflow). Formula shape mirrors cc-pool's: `service do … keep_alive true; run_at_load true; log_path ~/.cc-squash/daemon.log`, `ccs` symlink, `test do --version`. `brew install yasyf/tap/cc-squash`. Transient version skew between the two binaries is handled by the `Supervisor` version-skew replace (§2), **not** a separate mechanism. **The current PyPI/uvx model is retired** for the shipped artifact (see §9 supersession). `[verified — crates.io cargo-dist; cc-pool release.yml tap-publish]`
...
- **Considered and deferred (kept per-tool, recorded so the decision is durable).** (a) The **daemon socket-server scaffolding** stays per-tool: the bind is already `proc.SingleEntrant`, and the accept-loop + line-JSON proto-1 framing is thin while the message sets differ entirely (cc-pool's pool ops vs cc-squash's `mint`/`lookup`/`evict`/`shadow`/`kill`) — a shared `proc.Serve` abstraction over two unrelated protocols isn't worth it. (b) **Release/formula alignment** is orthogonal: cc-pool hand-rolls `release.yml`, cc-squash uses goreleaser, both publish to the shared `yasyf/homebrew-tap`; the duplicated `service do` block is generated config, not Go-lib code. `[inferred — dedup pass 2026-06-22]`
...
cc-squash/  (one repo: a Go module + a Rust workspace, co-released under one git tag)
  go/  (GO CONTROL PLANE — the `ccs` binary, cobra; consumes github.com/yasyf/fusekit v0.6.0 — proc + mountd + service + state + version)
...
- **Builds — Go half (control plane, fusekit):** `ccs url`/`env`/`run`/`status`/`stop`/`logs`/`service`/`daemon(hidden)`; the `~/.cc-squash/` state dir + `proc.SingleEntrant` single-entrant bind + atomic `daemon.port`/`status.json`; the unix control socket + the `proxy.sock` seam; the `proc.EnsureRunning` two-socket cold start; the per-session token **mint** (`OpMint`); the **two `proc.Supervisor` instances** (one per supervised child); launchd LaunchAgent + the single Homebrew formula (external tap, §1.5) via goreleaser + cargo-dist.
- **Builds — Rust half (data plane):** the `ccs-proxy` child — pingora **RelayCore** forwarding to `api.anthropic.com` with verbatim SSE passthrough; the first-party auth gate (`_CLAUDE_CODE_ASSUME_FIRST_PARTY_BASE_URL` + x-api-key/OAuth Bearer); `DashMap<SessionToken, SessionCtx>` demux by URL-path token; `ENABLE_TOOL_SEARCH=true` + `DISABLE_AUTO_COMPACT=1` asserted at startup; the kill-switch atomic; it **registers its port back** + reads `mint`/`shadow`/`kill` over `proxy.sock`.
...
- **Doc-consistency greps (the fusekit amendment):** `rg -n 'fuser|fuse3|fd-lock|fs2|flockSocket|ports cc-pool|verbatim to Rust' cc-squash-build-plan.md` returns only **explicitly-dropped** mentions (§4 "DROP …") or **Go-side / cc-pool-analog** references — no live instruction to port that layer to Rust survives; `rg -n 'fusekit|proc\.SingleEntrant|proc\.EnsureRunning|proc\.Supervisor|mountd|RO-CAS|proxy\.sock|goreleaser' cc-squash-build-plan.md` returns every new addition; and the §3 algorithm, the economics/policy crates, the pingora SSE relay, `retrieve()`, the eval harness, and **Appendix invariants 1–8 (and 9–12)** are byte-identical to the pre-amendment doc (`git diff`).
