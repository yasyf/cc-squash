...
> **Finalize pass (2026-06-20):** every crate in §4 was re-resolved against crates.io on this date — all are real and maintained; `pingora 0.8.1` and `cargo-dist/dist 0.32.0` match exactly. **(The fusekit amendment subsequently dropped the Rust FUSE crates `fuser`/`fuse3` from §4 — FUSE is now the fusekit Go holder, §2/§6.)** Two version corrections were folded in: **rmcp** is `1.7.0` (the SDK reached 1.x; the inherited `0.16` was stale) and **failsafe** is `1.3.0` (API-stable since Jul 2024). cc-pool's live state (`~/.cc-pool`: `daemon.sock`/`daemon.sock.lock`/`pool.db{,-wal,-shm}`/`mount-holder.log`/`mounts.sock`/`status.json`) and the superseded Python scaffold (`cc_squash/`, `pyproject.toml`, `uv.lock`) were both confirmed on disk, anchoring the §1 daemon ports and the §9 supersession pitfall.
...
- **launchd user-LaunchAgent with `KeepAlive`** (a **Go** port of cc-pool's `internal/service/` + `launchd/com.yasyf.cc-pool.plist.tmpl` — same language now, a direct port, not a Rust re-port): `ccs service install` writes `~/Library/LaunchAgents/com.<owner>.cc-squash.plist` (`RunAtLoad`, `KeepAlive`, `ThrottleInterval 10`, `ProcessType Background`, log to `~/.cc-squash/daemon.log`). A *user* agent (per-user state, localhost-only). `ccs url` auto-spawn covers the not-yet-installed case so it works with zero setup; the LaunchAgent just makes it survive reboots. `[verified — cc-pool service.go:20-67]`
...
cc-squash/  (one repo: a Go module + a Rust workspace, co-released under one git tag)
  go/  (GO CONTROL PLANE — the `ccs` binary, cobra; consumes github.com/yasyf/fusekit v0.5.0)
    cmd/ccs/            url, env, run, status, stop, logs, shadow, kill, gc, doctor, service, daemon(hidden), mount-holder(hidden)
    control/           proc.SingleEntrant bind, proc.EnsureRunning auto-launch, status.json/daemon.port atomic mirrors, OpMint, the Go↔Rust RPC seam (proxy.sock)
    service/           launchd / brew-services lifecycle (Go port of cc-pool internal/service/ + plist tmpl — stays Go)
