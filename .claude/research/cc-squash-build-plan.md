...
- **✅ AS-BUILT (Layer 2 DONE — on `main` @`21aab439`, CI-green, 148 tests, `/codex`-verified cost math).** Scope shipped was **pure crates only** (user-confirmed): the config-seam plumbing + `SessionCtx` `CacheState` field + the SSE usage tap are **deferred to Layer 4** where they're consumed (the "Entry points" / "Open design note" paragraph below is now Layer-4 guidance, not Layer-2 work). **Three crates** (the §2 two plus a leaf): `ccs-core` (branded newtypes + `SegmentKind`/`ChoiceTag`/`LineRange` + char-proxy estimator) → `ccs-economics` → `ccs-policy`; the `BatchView` trait (in economics, impl'd by `SquashBatch` in policy) breaks the would-be cycle. Determinism is total — every time-dependent fn takes `now: f64`; no clock/rand/I/O (grep-gated). **Five flagged-and-sound deviations from the §3 sketch** (all carried into Layer 3/4): (1) **`ccs-core` added** — `RefId` exposes only `parse("sha256:"+64hex)`, **no content-hashing ctor** (`ccs-refs` is the sole writer, Layer 3). (2) **`is_true_human` on the WIRE = `role==user ∧ content is a JSON string`** — the wire body does **not** carry `origin.kind`/`promptSource`/`isMeta`/`userType` (those are transcript-only; the `spec_keep` classifier is Layer 5's enrichment). `wire::WireMessage.content` is a `MessageContent{Text|Blocks}` enum so a string `content` actually deserializes. (3) **`budget::default_compact` returns `CompactionPlan{strip:Vec<usize>, dropped:Vec<usize>}`** (NOT `Vec<Strategy>` — `Strategy` can't express "shed thinking, keep the rest") and **takes `&WireBody`** (a `Segment` carries no content blocks; it correlates `seg.source_uuids → body.messages`). `strip_reasoning` branches on **both** `thinking` AND `redacted_thinking`, historical-only. (4) **`select_strategy` size-gates work in TOKEN space** via `seg.token_estimate` (`approx_chars = token_estimate·3.5`); the **huge-paste exception PRECEDES the pin** → `{Keep, ReversibleRef}` only. (5) **`PromptState` gained `free_bust: Option<FreeBustTrigger>`** — Layer 4 detects model-switch / native-compaction off-wire and feeds it; `Controller::decide` matches `(status, free_bust)` to recover the trigger without `unwrap`. **Open Layer-4 seams** (codex-flagged, deliberate): `EconomicsConfig.npv_floor` is defined-but-unread (both gates hardcode `npv > 0.0`, == the default; Layer 4 wires it via `mint{token,config}`); `break_even_turns` is spec-faithful but informational-only (never gated on — `npv` handles `T≤0` correctly). As-built note: cc-notes `743d0b0`; design doc: cc-notes `2f31bdf`.
- **Entry points (→ now Layer-4 guidance — where the engine plugs in):** the Interceptor replaces the **`decide(&bytes)` call site in `relay::serve`** (`crates/ccs-proxy/src/relay.rs`); it returns `Option<RewrittenRequest>` (None ⇒ forward original), mirroring the synth `Decision` short-circuit. **Wrap it in `tokio::time::timeout` → forward-original on timeout** (`CatchPanicLayer` already covers a panic). The request body is **already fully buffered as `Bytes`** before `decide()`, so segmentation/rewrite has the whole body with no streaming concern (only the *response* streams). `AppState` already carries `config: Arc<ArcSwap<RelayConfig>>` (lock-free hot-path read) + per-session `SessionCtx` in `DashMap<SessionToken, SessionCtx>` — Layer 2 fills the currently-empty `RelayConfig`/`SessionCtx` with the economics/policy config + per-session `CacheState`; the Go daemon is the SOLE `config.toml` parser and pushes the slice over the seam via `mint{token,config}`. **Open design note:** L0 needs the prior response's `usage` to refresh `CacheState`, but the relay currently streams the response untouched via `Body::from_stream` — Layer 2 must **tap** the SSE stream off-path (parse `message_start`/`message_delta` `usage`) without breaking verbatim passthrough. `ccs shadow {on}` (compute-but-forward) is already wired as the L1/eval capture substrate.
...
- **Consumes from Layer 2 (contracts to honor — these are now real on `main`):** (1) **`ccs-refs` owns the `RefId` constructor Layer 2 deliberately lacks** — `ccs_core::RefId` has only `parse(&str)`; `ccs-refs::put` is the SOLE place that hashes bytes → `RefId` (`sha256:<64hex>`), so add the content-addressing ctor here (do NOT add one to `ccs-core`). (2) **`select_strategy`/`Controller` already emit `Strategy::ReversibleRef{ ref_id, summary }` and `SquashCandidate{ ref_id, … }`** — Layer 3's L1 scorer is what *supplies* that `ref_id` (from `put`) and the `ContentDecision.summary_content`; Layer 2 only relays them. So the L1 pipeline is: `segment_prompt` (done) → score/`is_squash_candidate` → for survivors call the summarizer → `ContentDecision` (done: `normalize`+`pre_gate`) → `select_strategy` (done). (3) **`WorkingState` is an INPUT to `ccs_policy::is_pinned`** — Layer 3's Rsum folder (§3c) is what builds it; `Constraint.source_message`/`superseded_by` already drive the bi-temporal pin (note: in Layer 2 only `Constraint` pins by `source_message`; `Decision`/`InFlightWork` lack that field — wire it if Layer 3 needs them to pin specific segments). (4) **Dedup payload-hash key (§3d) == the `ccs-refs` content-address** — one hash feeds both; assert tool-pair integrity separately (a dedup-collapse must never break a `TOOL_PAIR`). (5) **The placeholder render + `REF_MARKER` regex must round-trip** what `Strategy::ReversibleRef` carries (`ref_id` + `summary`); the live byte-application of that strategy is Layer 4, so Layer 3 ships the renderer + the `materialize` reader, not the wire rewrite.
