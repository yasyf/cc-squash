...
func TestInjectRun(t *testing.T) {
	cases := map[string]struct {
		...
		"--help is ccs's own":           {[]string{"--help"}, []string{"--help"}},
		"-h is ccs's own":               {[]string{"-h"}, []string{"-h"}},
		"--version is ccs's own":        {[]string{"--version"}, []string{"--version"}},
		"-v is ccs's own":               {[]string{"-v"}, []string{"-v"}},
